...
f_sics<-function(model,items,individuals,s_count){
  ...
  if(file.exists(testfile)){
    ...
    ret$probability_matrix = ret$est_items$probability_matrix
    ...
    #Estimar Individuos
    tm = proc.time();
    ret$eap = individual.traits(dataset=test$test,model=model,itempars=ret$est_items,
                                method="EAP",probability_matrix=ret$probability_matrix);
    tm = proc.time()-tm;
    ret$time_eap <- tm[3];
...
scenarios.run<- function(connection=mongo,db="test",scenario.collection="scenarios",out.collection="out"){
  ...
  repeat{
    ...
    query1 = paste0('{"s_status" : "notrun",  "s_count" : ',c_rep$value,' , "s_function" : "f_sics"}')
